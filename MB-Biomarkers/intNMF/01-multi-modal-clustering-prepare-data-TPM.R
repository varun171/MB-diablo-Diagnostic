...
# 1) read count data
tpm_file <- file.path(data_dir, "gene-expression-rsem-tpm_medullo-collapsed.rds")
tpm_mat <- readRDS(tpm_file)
tpm_mat <- tpm_mat %>%
  dplyr::select(any_of(histology_file$Kids_First_Biospecimen_ID))
tpm_mat <- melt(as.matrix(tpm_mat), varnames = c("Gene", "Kids_First_Biospecimen_ID")) 
...
# 4) Methylation
# read beta-values
methyl_data <- readRDS(file.path(data_dir, "methyl-m-values_medullo.rds"))
...
samples_of_interest <- intersect(colnames(tpm_mat), rownames(methyl_data))


# now final filter/transformation on samples of interest

# 1) RNA
tpm_mat <- t(tpm_mat) %>% as.data.frame()
tpm_mat <- tpm_mat[samples_of_interest,]
# remove genes with 0 counts across all samples
tpm_mat <- tpm_mat[,colSums(tpm_mat) > 0]  

# top 1000 most variable genes
num_genes = 1000
keep <- apply(tpm_mat, 2, var)
keep <- keep[rev(order(keep))[1:num_genes]]
keep <- unique(c(names(keep)))
tpm_mat <- tpm_mat[,colnames(tpm_mat) %in% keep]

# rank transformation
#count_mat <- ranktransform(t(count_mat) %>% as.data.frame())
#count_mat <- t(count_mat) %>% as.data.frame()
print(dim(tpm_mat)) # 1000
write_tsv(as.data.frame(tpm_mat) %>% rownames_to_column(), file = file.path(data_dir, "norm_tpm.tsv"))
...
write_tsv(as.data.frame(methyl_data) %>% rownames_to_column(), file = file.path(data_dir, "methyl_mdata.tsv"))

# final sample map
rna_samples <- tpm_samples %>%
...
write_tsv(sample_map, file = file.path(data_dir, "samples_newmap.tsv"))
