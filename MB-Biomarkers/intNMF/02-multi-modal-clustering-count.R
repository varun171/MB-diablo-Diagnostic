...
# read data
count_data <- read_tsv(file.path(data_dir, "norm_counts.tsv")) %>% column_to_rownames()
methyl_data <- read_tsv(file.path(data_dir, "methyl_data.tsv")) %>% column_to_rownames()
...
nmf_output <- nmf.mnnals(dat = dat, k = 5, maxiter = 200, st.count = 10, n.ini = 10, ini.nndsvd = TRUE, seed = TRUE,wt=if(is.list(dat)) rep(1,length(dat)) else 1)
