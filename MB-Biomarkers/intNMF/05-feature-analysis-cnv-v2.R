...
# read cnv data and extract features with non-zero NMF weights per cluster
...
for(i in 1:cluster_max){
  ...
  # GSOA
  set.seed(100)
  go_enrich[[i]] <- clusterProfiler::enrichGO(gene = cnv_genes,
                                              OrgDb = org.Hs.eg.db, 
                                              keyType = 'SYMBOL',
                                              readable = T,
                                              ont = "BP",
...
# Upset plot
pdf(file = file.path(plots_dir, "cnv_gsoa.pdf"), width = 12, onefile = TRUE)
for(i in 1:cluster_max){
...
# Word cloud 
pdf(file = file.path(plots_dir, "cnv_wordcloud.pdf"), onefile = TRUE)
for(i in 1:cluster_max){
  go_enrich_cluster <- go_enrich[[i]] %>% 
    as.data.frame()
  if(nrow(go_enrich_cluster) > 0){
...
#  ggplot by p-value
pdf(file = file.path(plots_dir, "cnv_barplot.pdf"), onefile = TRUE, width = 10, height = 12)
for(i in 1:cluster_max){
  print(i)
  # only plot top 100 significant because there are too many results
...
pdf(file = file.path(plots_dir, "cnv_barplot_graphics.pdf"), onefile = TRUE, width = 8, height = 6)
for(i in 1:cluster_max){
  print(i)
  go_enrich_cluster <- go_enrich[[i]] %>% 
    as.data.frame()
...
# Enrichplot 
pdf(file = file.path(plots_dir, "cnv_goplot.pdf"), onefile = TRUE, width = 14, height = 12)
for(i in 1:cluster_max){
  print(i)
  go_enrich_cluster <- go_enrich[[i]] %>% 
    as.data.frame()
