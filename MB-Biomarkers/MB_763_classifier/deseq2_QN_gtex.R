...
# Apply quantile normalization to GTEx data
normalized_counts_gtex <- normalize.quantiles(as.matrix(gtex_count_data))

# Convert back to data frame and add rownames
normalized_counts_gtex <- as.data.frame(normalized_counts_gtex)
rownames(normalized_counts_gtex) <- rownames(gtex_count_data)
colnames(normalized_counts_gtex) <- colnames(gtex_count_data)
...
pca_plot_before <- plot_pca(combined_expr_data, final_hist_df, "PCA Before Normalization", "rf_predicted_subtype")
print(pca_plot_before)
ggsave(file.path(plots_dir, "PCA_Before_Normalization.pdf"), plot = pca_plot_before)

# Plot PCA after quantile normalization
pca_plot_after <- plot_pca(combined_normalized_counts, final_hist_df, "PCA After Quantile Normalization", "rf_predicted_subtype")
print(pca_plot_after)
ggsave(file.path(plots_dir, "PCA_After_Quantile_Normalization.pdf"), plot = pca_plot_after)

# Round normalized counts to nearest integer
...
for (histology in unique_histologies) {
  ...
  output_file <- file.path(result_dir, paste0("DESeq2_quantile_results_", histology, "_vs_Normal_Brain.csv"))
...
grp3_alpha_DE <- read_csv(file.path(result_dir, "DESeq2_quantile_results_subtype: Group3_alpha_vs_Normal_Brain.csv"))
grp3_beta_DE <- read_csv(file.path(result_dir, "DESeq2_quantile_results_subtype: Group3_beta_vs_Normal_Brain.csv"))
grp3_gamma_DE <- read_csv(file.path(result_dir, "DESeq2_quantile_results_subtype: Group3_gamma_vs_Normal_Brain.csv"))
