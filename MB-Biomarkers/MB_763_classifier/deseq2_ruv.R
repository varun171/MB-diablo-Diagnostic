...
# Apply RUVSeq for removing unwanted variation
set <- newSeqExpressionSet(as.matrix(combined_expr_data),
                           phenoData = data.frame(final_hist_df, row.names = colnames(combined_expr_data)))
...
for (k_val in 1:5) {
  set_k <- RUVg(set, rownames(combined_expr_data), k = k_val)
  normalized_counts_k <- counts(set_k)
  pca_plot_after <- plot_pca(normalized_counts_k, final_hist_df, paste("PCA After RUV (k =", k_val, ")"), "rf_predicted_subtype")
  print(pca_plot_after)
  ggsave(file.path(plots_dir, paste0("PCA_After_RUV_k_", k_val, ".pdf")), plot = pca_plot_after)
}

# Apply chosen RUVg normalization (example with k=1)
set <- RUVg(set, rownames(combined_expr_data), k = 1)
normalized_counts <- counts(set)
...
for (histology in unique_histologies) {
  ...
  expr_data_subset <- normalized_counts[, final_hist_df_subset$Kids_First_Biospecimen_ID]
