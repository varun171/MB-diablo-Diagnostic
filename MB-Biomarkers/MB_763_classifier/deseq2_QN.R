...
# Plot PCA before quantile normalization
pca_plot_before <- plot_pca(combined_expr_data, final_hist_df, "PCA Before Quantile Normalization", "rf_predicted_subtype")
print(pca_plot_before)
ggsave(file.path(plots_dir, "PCA_Before_Quantile_Normalization.pdf"), plot = pca_plot_before)

# Apply quantile normalization
normalized_counts <- normalize.quantiles(as.matrix(combined_expr_data))

# Convert back to data frame and add rownames
normalized_counts <- as.data.frame(normalized_counts)
rownames(normalized_counts) <- rownames(combined_expr_data)
colnames(normalized_counts) <- colnames(combined_expr_data)

# Plot PCA after quantile normalization
pca_plot_after <- plot_pca(normalized_counts, final_hist_df, "PCA After Quantile Normalization", "rf_predicted_subtype")
print(pca_plot_after)
ggsave(file.path(plots_dir, "PCA_After_Quantile_Normalization.pdf"), plot = pca_plot_after)

# Convert quantile normalized counts to integers (not recommended due to potential bias)
integer_normalized_counts <- round(normalized_counts)

# Differential expression analysis using rounded quantile normalized counts
...
for (histology in unique_histologies) {
  ...
  expr_data_subset <- integer_normalized_counts[, final_hist_df_subset$Kids_First_Biospecimen_ID]
  ...
  output_file <- file.path(result_dir, paste0("DESeq2_QN_results_", histology, "_vs_Normal_Brain.csv"))
...
grp3_alpha_DE <- read_csv(file.path(result_dir, "DESeq2_QN_results_subtype: Group3_alpha_vs_Normal_Brain.csv"))
grp3_beta_DE <- read_csv(file.path(result_dir, "DESeq2_QN_results_subtype: Group3_beta_vs_Normal_Brain.csv"))
grp3_gamma_DE <- read_csv(file.path(result_dir, "DESeq2_QN_results_subtype: Group3_gamma_vs_Normal_Brain.csv"))
